...
# global args
args.save="/home/sunyi/MetaGF_TPAMI/tmpsave/sparsification_t1_2022-9-8-10-20-42/"
...
def weighting_themodel(model,weightmodel,task,threshold):
    ##soft weighting the routes

    # dynamically soft-pruning the network
    model_dict = deepcopy(model.state_dict())
    for name in model_dict:
        if weightmodel.module.weightlist[task].gradlist.__contains__(name):
            gate= torch.sigmoid(args.temperature*getattr(weightmodel.module.weightlist[task], name.replace(".", "#")).w)
            gate[gate>threshold]=1
            gate[gate<=threshold]=0*gate[gate<=threshold]
            model_dict[name] = model_dict[name] * expand(gate,model_dict[name])
            # gate = torch.sigmoid(
            #     args.temperature * getattr(weightmodel.module.weightlist[task], name.replace(".", "#")).w)
            # model_dict[name] = F.normalize(model_dict[name], dim=1) * expand(gate, model_dict[name])
...
def main(pruningdepth):
    ...
    weighting_themodel(model,weight_model,pruningdepth,threshold=threshold)

    totalnum = 0
    pruned = 0
    for name in weight_model.module.weightlist[pruningdepth].gradlist:
        w = torch.sigmoid(args.temperature * (
            getattr(weight_model.module.weightlist[pruningdepth], name.replace(".", "#")).w)).view(-1)
        mask = w < threshold
        pruned += mask.sum()
        totalnum += w.shape[0]

    print("current sparsification ratio:{0}".format(pruned / totalnum))
