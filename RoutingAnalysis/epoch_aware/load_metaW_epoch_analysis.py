...
from G0_Gradient_tools.bk.MetaGFgradBN import Meta_fusion_weights_list
...
def sy_load_static():
    ...
    args.data='cifar10'
    args.task=args.data
    folder="/home/sunyi/failed/save_models/"
    ...
    with torch.no_grad():
        for name in model.state_dict():
            ...
            for epochidx in range(0,100,1):
                static_name=folder+'routingindex_{0}.pth.tar'.format(epochidx)
                if not os.path.exists(static_name):
                    continue
                dict_model = torch.load(static_name, map_location="cuda:0")
                ...
                for i in range(0, len(weight_model.module.weightlist)):
                    if weight_model.module.weightlist[i].gradlist.__contains__(name):
                        ...
                        if p.dim() == 4:
                            w_ = torch.abs(getattr(weight_model.module.weightlist[i], name.replace(".", "#")).w)

                            w_norm_relative = w_ / (1e-4 * torch.sum(w_, dim=1, keepdim=True) + 1).pow(0.75)

                            tmp_sum += w_norm_relative
                            # w_expand = expand(w, shared_parameter)
                        elif p.dim() == 1:
                            w_ = torch.sigmoid(getattr(weight_model.module.weightlist[i], name.replace(".", "#")).w)
                ...
                for i in range(0,len(weight_model.module.weightlist)):
                    if weight_model.module.weightlist[i].gradlist.__contains__(name):
                        if p.dim() == 4:
                            w_ = torch.abs(getattr(weight_model.module.weightlist[i], name.replace(".", "#")).w)
                            # fusion_mag = tmpconnection - getattr(weight_model.module.thresholdlist[i],
                            #                                      name.replace(".", "#")).threshold
                            # mask = (fusion_mag) > 0  # channel-wise pruning
                            # w_ = tmpconnection * mask
                            # w_norm=torch.sum(w_,dim=1,keepdim=True)
                ...
                fig = plt.figure(1)
                plt.clf()
                plt.title(name+"_epoch:{}".format(epochidx))
                ...
                # bar
                # bar
                downsample_ratio=shared_matrix.shape[1]//16
