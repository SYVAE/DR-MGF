...
class Meta_fusion_weights(nn.Module):
    def __init__(self, oldmodel, depth,SCALINGvalue,SCALING=False):
        ...
        '''Noting!!!! an advice for future work.
           going to do: you can use dict to store the importance weights of each neuron, for efficient and brief indexing
           e.g, 
           replacing:
                self.add_module(name,one_weigth_module(p.shape[0], p.shape[1], require_grad=True, magnitude=SCALINGvalue))
           with:
                self.weight[p.weight]=one_weigth_module(p.shape[0], p.shape[1], require_grad=True, magnitude=SCALINGvalue)
           '''
...
class MetaGrad():
    ...
    def adapt_model(self, model, weightmodel, Grad_Dictlist, train_loader, epoch):
        ...
        progress_bar = tqdm(range(train_batch),desc=f'Meta-fusiong (epoch {epoch})')
                    ...
                    if name in self.Conbiasnamelist: #shared bias
                        ...
                        for idx in range(0, self.tasknum):
                            ...
                            if name.__contains__('weight'):
                                tmpconnection = torch.abs(getattr(weightmodel.module.weightlist[idx], name.replace(".", "#")).w).squeeze()
                                tmpconnection = tmpconnection / (torch.sum(tmpconnection, dim=0, keepdim=True) + 1e-10).pow(1)
                                tmpconnection = tmpconnection * torch.abs(
                                getattr(weightmodel.module.scalinglist[idx], name.replace(".", "#")).w).squeeze()

                            else:
                                tmpconnection = torch.abs(
                                    getattr(weightmodel.module.scalinglist[idx], name.replace(".", "#")).w).squeeze()
                                # else:
                            #     tmpconnection = torch.sigmoid(getattr(weightmodel.module.weightlist[idx], name.replace(".", "#")).w).squeeze()
                            w_ = tmpconnection
                            # torch.abs(getattr(weightmodel.module.scalinglist[idx], name.replace(".", "#")).w).squeeze()
