...
def meta_fusingModel(model, weightmodel, tasknum):
    '''metaupdating now'''
    ...
    for name, p in model.named_parameters():
        ...
        if p.dim() == 4:
            weightslist = []
            if (not weightmodel.module.weightlist[tasknum].forwardlist.__contains__(name)):
                pass
            else:
                if getattr(weightmodel.module.weightlist[tasknum], name.replace(".", "#")).w is None:
                    meta_connection_weight[name] = None
                else:
                    shared += 1
                    '''calculating gradient'''
                    # #pruned-structure
                    tmpconnection = torch.abs(getattr(weightmodel.module.weightlist[tasknum], name.replace(".", "#")).w)
                    p = tmpconnection / (1e-4 * torch.sum(tmpconnection, dim=1, keepdim=True) + 1.0).pow(0.75)
                    meta_connection_weight[name] = p
                ...
                if getattr(weightmodel.module.weightlist[tasknum], name.replace(".", "#")).w is None:
                    ...
                    if name.__contains__('weight'):
                        tmpconnection = torch.abs(
                            getattr(weightmodel.module.weightlist[tasknum], name.replace(".", "#")).w)
                        # tmpconnection = tmpconnection / (1e-4 * torch.sum(tmpconnection, dim=0, keepdim=True) + 1.0).pow(0.75)
                    else:
                        tmpconnection = getattr(weightmodel.module.weightlist[tasknum], name.replace(".", "#")).w
                    # w_ = w_ / (1e-4 * torch.sum(w_, dim=0, keepdim=True) + 1).pow(0.75)

                    meta_connection_weight[name] = tmpconnection
        ...
        if weightmodel.__contains__(name):
            ...
            if model_dict[name].dim() == 4:
                if w is not None:
                    tmpconnection = w
                    ##1-3
                    # fusion_mag = tmpconnection - getattr(weightmodel.module.thresholdlist[task],
                    #                                      name.replace(".", "#")).threshold
                    # mask = (fusion_mag) > 0
                    gate = tmpconnection

                    # gate = gate / (1e-4 * torch.sum(gate, dim=1, keepdim=True) + 1.0).pow(0.75)
                ...
                if w is not None:
                    if name.__contains__('weight'):
                        ...
                        tmpconnection = (w).squeeze()
                    nanmask = torch.isnan(tmpconnection)
                    if nanmask.sum():
                        raise ("nan detected")
                    model_dict[name] = tmpconnection
                else:
                    pass
...
if __name__ == '__main__':
    ...
    taskmodel_optimizer1=optim.Adam(SegNet_MTAN.parameters(), lr=opt.lr)
    scheduler1 = optim.lr_scheduler.StepLR(taskmodel_optimizer1, step_size=100, gamma=0.5)



    optimizer = MetaGrad(optimizer=taskmodel_optimizer1, temperature=1,
                         tasknum=TASKNUM,
                         meta_Optimizer=stepsizeOptimizer, inneriteration=1,device=device)
    ...
    for epoch in range(0,200):
        ...
        for taskid in tqdm(range(0,TASKNUM)):
            train_minibatch(nyuv2_train_loader, SegNet_MTAN, device,
                        optimizer,  weight_model, epoch, taskid, lambda_weight,taskmodel_optimizer1)
