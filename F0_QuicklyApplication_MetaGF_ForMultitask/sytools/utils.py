...
class KDLoss(nn.Module):
    ...
    def run(self, outputs, soft_targets):
        loss = 0
        T = self.T
        for i in range(self.nBlocks - 1):
            _kld = self.kld_loss(self.log_softmax(outputs[i] / T), self.softmax(soft_targets.detach() / T)) * self.gamma * T * T
    ...
    def run_weighting(self, outputs, soft_targets,task,ratio):
        loss = 0
        T = self.T
        for i in range(self.nBlocks - 1):
            if i != task:
                tmp =ratio[i]* self.kld_loss(self.log_softmax(outputs[i] / T), self.softmax(soft_targets.detach() / T)) * self.gamma * T * T
                loss += tmp
            else:
                tmp = self.kld_loss(self.log_softmax(outputs[i] / T),
                                               self.softmax(soft_targets.detach() / T)) * self.gamma * T * T
                loss += tmp
        return loss

    def loss_kd_1to1(self,students,teachers):
        T = self.T
        _kld = self.kld_loss(self.log_softmax(students / T),
                             self.softmax(teachers / T)) * self.gamma * T * T
        return _kld
