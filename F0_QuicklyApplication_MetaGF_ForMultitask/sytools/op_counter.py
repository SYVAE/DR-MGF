...
import operator
from tools.Sparse_conv import *
# from architectures.SDNs.ResNet_SDN_softpruning import SparseConv
...
def measure_layer(layer, x):
    ...
    ### ops_conv
    if type_name in ['Conv2d','SparseConv']:
    ...
    ### ops_linear
    elif type_name in ['Linear','SparseLinear']:
    ...
    ### ops_nothing
                       ...
                       'MSDNLayer', 'ConvDownNormal', 'ConvNormal', 'ClassifierModule','Flatten','Identity','Sparse_BN']:
        delta_params = get_layer_param(layer)
    ...
    if type_name == 'Linear' or type_name=='SparseLinear':
        print('---------------------')
            ...
            if should_measure(child):
                def new_forward(m):
                    if isinstance(m,SparseConv):
                        def lambda_forward(x,fusionweight=None):
                            measure_layer(m, x)
